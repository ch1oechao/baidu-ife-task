...
function listCates(arr){
    ...
    delegateClickEvent(init.todoTaskItem, taskItemClick);
    //刷新未完成任务总数
    init.todoTotal.innerHTML = "("
                               + todoCount("data.tasks", "all")
                               + ")";
    //更新内容视图，默认任务内容
    addContent(data.tasks[0]);
}
...
function addCatePanel(display){
    ...
    if (addCatePanel) {
        ...
        if(cWidth > 768){
            ...
            addCatePanel.style.left = Math.round(cWidth / 4.5) + "px";
        }
        else if(cWidth > 400 && cWidth <= 640){
            addCatePanel.style.left = Math.round(cWidth / 7.5) + "px";
        }
        else if(cWidth > 350 && cWidth <=400){
            addCatePanel.style.left = 20 + "px";
        }else{
            addCatePanel.style.left = 0 + "px";
        }
 ...
 * 编辑任务图标切换
 *  
 * @param {String} status 传入内容视图的状态，编辑状态或显示状态
 */
